---
title: "Sentiment Analysis : Top 30 Burger Joints in Thailand"
...
- Step 7 - Sentiment Scores & Emotion Classification

## Step 1 - Problem Definition

To identify the top 30 **Burger Joints** among the given tourist accommodations and analyze the sentiment by restaurant and produce an overall emotional feedback for all the burger joints as a whole. 
...
1. Setup the positive & negative lexicon for Sentiment Analysis
2. Import the Tourist Accommodation reviews file into R
3. Inspect the files
...
1. Duplicate (to lowercase) the "review" column - to transform it later
2. Identify the "burger joints" & Filter them out as a subset
3. Inspect the new subset
4. Inspect the review column to validate the search
5. Creating a function for sentimental analysis
6. Creating a function for Word Cloud
...
for (burg_joint in v_burgJoints) {
  ...
  # 5.3.9 - Replace "<*>" any markup texts with a space from the reviews
  vec_burg_reviews <- gsub("<*>", " ", vec_burg_reviews)

  # 5.3.10 - Converting the text vectors to corpus
  corpus_burg_revs <- Corpus(VectorSource(vec_burg_reviews))
  
  # 5.3.11 - Clean up corpus by removing stop words and Whitespace
  ...
  # 5.3.12 - Stem the words to their root of all reviews present in the corpus
  stem_corpus_burg_revs <- tm_map(corpus_burg_revs, stemDocument)
  
  # 5.3.13 - Utilize the sentiment analysis function for the stemmed corpus and append it to a dataframe
...
for (burg_joint in v_top30_burg_jts) {
  ...
  # 5.3.8 - Replace "<*>" any markup texts with a space from the reviews
  vec_burg_reviews <- gsub("<*>", " ", vec_burg_reviews)
  
  # 5.9.9 - Converting the text vectors to corpus
  corpus_burg_revs <- Corpus(VectorSource(vec_burg_reviews))
  
  # 5.9.10 - Clean up corpus by removing stop words and Whitespace
  corpus_burg_revs <- tm_map(corpus_burg_revs, removeWords,stopwords("english"))
  corpus_burg_revs <- tm_map(corpus_burg_revs, stripWhitespace)
  
  # 5.9.11 - Stem the words to their root of all reviews present in the corpus
  stem_corpus_burg_revs <- tm_map(corpus_burg_revs, stemDocument)
  
  # 5.9.12 - Execute Word Cloud function for the Stem Corpus
  f_wc(stem_corpus_burg_revs, burg_joint)
}
```

10. Saving the cleaned dataset as a .csv (for SAS)
```{r echo=TRUE}
# 4.1 - Write to a .csv file
write.xlsx(df_pop30_revs[1:5], "Thai_BurgerJoints.xlsx", row.names=TRUE)
...
## Step 7 - Overall Sentiment Scores & Emotion Classification

1. Scales to measure sentiment
```{r echo = TRUE}
...
3. Data Transformation for Emotions
```{r echo = TRUE}
# 7.3.1 - Transpose
